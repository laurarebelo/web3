...
describe("Hand set up", () => {
  ...
  it("has the players set in the properties", () => {
    expect(hand.player(0).name).toBe('a')
    expect(hand.player(1).name).toBe('b')
    expect(hand.player(2).name).toBe('c')
    expect(hand.player(3).name).toBe('d')
  })
  ...
  it("keeps shuffling as long as the top of the discard pile is a wild card", () => {
    ...
    console.log(`creating hand for 14...`)
    const hand = createHand({players: ['a', 'b', 'c', 'd'], dealer: 1, shuffler})
    expect(mockShuffler).toBeCalledTimes(1)
  })
